...
/* ── Inline Daily Check-In (embedded in profile menu) ── */
function InlineDailyCheckIn({ userId, onClaimed }: { userId: string; onClaimed: () => void }) {
  ...
  const { data: status, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as {
        ...
        next_day: number; // 1–7, wraps back to 1 after day 7
  ...
  // next_day is 1–7 and wraps back to 1 after completing day 7
  const nextDay = status?.next_day ?? 1;
  // dayIndex is 0-based for array access — wraps correctly via modulo
  const dayIndex = ((nextDay - 1) % 7 + 7) % 7;
  const todayReward = CHECKIN_REWARDS[dayIndex];
  const streak = status?.current_streak ?? 0;

  // How many days in the current 7-day cycle have been claimed
  // streak % 7 === 0 means either no days claimed yet, or just completed a full cycle
    ...
    <div className="px-4 py-3 border-b border-white/10">
      ...
      <div className="grid grid-cols-7 gap-1 mb-3">
        {CHECKIN_REWARDS.map((r, i) => {
          const dayNum = i + 1;
          // A day is claimed if it falls within the current cycle's claimed days
          const isClaimed = dayNum <= daysClaimedInCycle;
          // Today's claimable day — only if can_check_in and streak hasn't consumed it yet
            ...
            <div
              ...
              className={`relative flex flex-col items-center gap-0.5 rounded-lg border p-1 transition-all ${
                isClaimed
                  ...
                  : isToday
                  ? "border-yellow-400/40 bg-yellow-400/10 shadow-[0_0_8px_rgba(250,204,21,0.2)]"
...
/* ── Profile Menu — Inventory replaced with inline Daily Check-In ── */
    ...
    <div className="relative">
      ...
      >
        {full?.discord_avatar ? (
          ...
          <div
            className={`w-7 h-7 rounded-lg border ${el?.border ?? "border-white/20"} bg-white/10 flex items-center justify-center text-xs font-bold text-white`}
          >
          ...
          >
            ...
            <div className="p-4 border-b border-white/10 flex items-center gap-3">
              {full?.discord_avatar ? (
                ...
                <div
                  className={`w-14 h-14 rounded-xl border-2 ${el?.border ?? "border-white/20"} bg-white/10 flex items-center justify-center text-xl font-bold text-white`}
                >
                  {username.charAt(0).toUpperCase()}
                </div>
              )}
            ...
            {/* INLINE DAILY CHECK-IN (replaces inventory) */}
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 p-2.5 rounded-lg border ${
                isMyGuild ? el.border : "border-zinc-800"
              } ${isMyGuild ? el.bg : "bg-zinc-900/20"}`}
            >
              <div
...
function GuideSection() {
  const guides = [
    {
      icon: Swords, title: "Attack",
      desc: "Use Nuke (100MP), Drain (25MP), or Rug (50MP) to weaken enemy guilds. Items come from mystery chests.",
      color: "text-red-400", border: "border-red-500/20", bg: "bg-red-950/10",
    },
    {
      icon: Shield, title: "Defend",
      desc: "Activate Shield to block all attacks for 24h. Use HP Potions to restore 10% guild HP. 5h cooldown per item type.",
      color: "text-blue-400", border: "border-blue-500/20", bg: "bg-blue-950/10",
    },
    {
      icon: Zap, title: "MP System",
      desc: "MP starts at 100 and regenerates fully over 48 hours. MP Potions restore instantly. Plan your attacks wisely.",
      color: "text-yellow-400", border: "border-yellow-500/20", bg: "bg-yellow-950/10",
    },
    {
      icon: Gem, title: "Crafting",
      desc: "Collect 4 identical shards to craft 1 elemental. Gather 6 elementals to unlock wallet submission for GTD.",
      color: "text-purple-400", border: "border-purple-500/20", bg: "bg-purple-950/10",
    },
    {
      icon: Trophy, title: "Ranking",
      desc: "Guilds are ranked by total member coins + guild score. Daily check-ins add to your guild's score. Contribute to climb.",
      color: "text-green-400", border: "border-green-500/20", bg: "bg-green-950/10",
    },
    {
      icon: Sparkles, title: "Mystery Chest",
      desc: "Open every 2 hours for free. Drops: Coins (50%), Shards (25%), Items (20%), Elementals (5%).",
    ...
    <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 md:p-8">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-zinc-500">
          Master the battlefield. Every action shapes your guild's fate.
        </p>
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="min-h-[80vh] flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          >
            ...
            {el ? (
              <p className={`text-xs uppercase tracking-[0.2em] ${el.text} mb-3`}>
                {el.label} element bound
              </p>
            ) : (
            ...
            <p className="mt-4 text-white/45 text-sm leading-relaxed max-w-xs mx-auto">
              The 20 guilds have been selected. The protocol has begun. Fight for your guild, climb
              the rankings, and claim glory.
            </p>
            ...
            <div
              className={`mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-full border ${
                el?.border ?? "border-white/10"
              } ${el?.bg ?? "bg-white/5"} backdrop-blur-md text-sm`}
            >
        ...
        <div className="px-4 sm:px-8 py-12 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              ...
              {
                label: "Your Coins",
                value: (fullProfile?.coin_balance ?? fullProfile?.coins ?? 0).toLocaleString(),
                icon: () => <img src={GAME_ASSETS.coin} className="w-4 h-4" />,
                color: "text-yellow-400",
              },
              {
                label: "Guild HP",
                value: `${fullProfile?.guild_hp ?? "—"}%`,
                icon: Heart,
                color: "text-red-400",
              },
              {
                label: "Check-in Streak",
                value: `${checkInStatus?.current_streak ?? 0} days`,
                icon: Flame,
