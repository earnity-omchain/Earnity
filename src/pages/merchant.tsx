...
import {
  ...
  ArrowRightLeft, LayoutList, LayoutGrid, Send,
} from "lucide-react";
import {
  GAME_ASSETS,
  ELEMENTAL_IMAGES,
  ...
  return (
    <motion.div layout
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/8">
        ...
        {isMine ? (
          <button onClick={() => onCancel(listing.id)} disabled={isPendingCancel}
          ...
          <button onClick={() => onBuy(listing)} disabled={isPendingBuy || !canAfford}
            className={`px-3 py-1.5 rounded-xl border text-xs font-semibold transition-colors disabled:opacity-40 flex items-center gap-1.5 ${
              canAfford ? "bg-white/10 hover:bg-white/20 border-white/15 text-white" : "bg-white/5 border-white/10 text-white/30"
            }`}>
      ...
      <div className="flex items-center gap-2 pt-1 border-t border-white/5">
        ...
        {isMine && <span className="ml-auto text-[10px] text-yellow-400/60 font-mono">Your listing</span>}
      </div>
    </motion.div>
  );
}
...
function ListingCardGrid({ listing, isMine, coinBalance, onBuy, onCancel, isPendingBuy, isPendingCancel }: any) {
  const meta      = getItemMeta(listing.item_type);
  const canAfford = coinBalance >= listing.price_coins;
  return (
    <motion.div layout
      initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }}
      className={`rounded-2xl border ${meta.border} bg-black/60 backdrop-blur-md p-3 flex flex-col gap-2`}>
      <div className="flex items-start justify-between">
        <ItemVisual itemType={listing.item_type} size="md" />
        <span className="text-[10px] font-mono font-bold text-white/40 bg-white/5 border border-white/8 px-1.5 py-0.5 rounded-md">
          x{listing.quantity}
        </span>
      </div>
      <div>
        <div className="font-semibold text-white text-xs truncate">{meta.name}</div>
        <div className="flex items-center gap-1 mt-1">
          <Coins className="w-3 h-3 text-yellow-400" />
          <span className="text-xs font-black text-white font-mono">{listing.price_coins.toLocaleString()}</span>
        </div>
      </div>
      <div className="flex items-center gap-1.5 pt-1 border-t border-white/5">
        {listing.seller?.discord_avatar
          ? <img src={listing.seller.discord_avatar} className="w-3.5 h-3.5 rounded-full object-cover border border-white/10" alt="" />
          : <div className="w-3.5 h-3.5 rounded-full bg-white/10 flex items-center justify-center text-[8px] font-bold text-white/50">
              {(listing.seller?.username || "?").charAt(0).toUpperCase()}
            </div>
        }
      ...
      {isMine ? (
        <button onClick={() => onCancel(listing.id)} disabled={isPendingCancel}
          className="w-full py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-[10px] text-red-400 font-medium transition-colors disabled:opacity-40 flex items-center justify-center gap-1">
          {isPendingCancel ? <Loader2 className="w-3 h-3 animate-spin" /> : <X className="w-3 h-3" />}
          Cancel
        </button>
      ) : (
        <button onClick={() => onBuy(listing)} disabled={isPendingBuy || !canAfford}
          className={`w-full py-1.5 rounded-lg border text-[10px] font-semibold transition-colors disabled:opacity-40 flex items-center justify-center gap-1 ${
            canAfford ? "bg-white/10 hover:bg-white/20 border-white/15 text-white" : "bg-white/5 border-white/8 text-white/30"
          }`}>
          {isPendingBuy ? <Loader2 className="w-3 h-3 animate-spin" /> : <ShoppingBag className="w-3 h-3" />}
          {canAfford ? "Buy" : "Can't afford"}
        </button>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-[10px] text-white/40 mb-1.5 uppercase tracking-wider font-mono">Price (coins)</label>
          ...
          <button onClick={() => canSubmit && !isPending && onSubmit(parsedQty, parsedPrice)}
...
/* ── Transfer modal ── */
function TransferModal({ item, onClose, onSubmit, isPending }: {
  item: any; onClose: () => void;
  onSubmit: (qty: number, username: string) => void;
  isPending: boolean;
}) {
  const [qty, setQty]           = useState("1");
  ...
  const meta                    = getItemMeta(item.item_type);
  const parsedQty               = Math.min(Math.max(1, parseInt(qty) || 1), item.quantity);
  const canSubmit               = parsedQty > 0 && username.trim().length > 0;
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
      onClick={onClose}>
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-white">Transfer Item</h3>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className={`rounded-xl border ${meta.border} ${meta.bg} p-3 mb-4 flex items-center gap-3`}>
          <ItemVisual itemType={item.item_type} size="sm" />
          <div>
            <div className="font-semibold text-white text-sm">{meta.name}</div>
            <div className="text-[11px] text-white/40">Available: {item.quantity}</div>
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-cyan-500/8 border border-cyan-500/20 mb-4">
          <Send className="w-3.5 h-3.5 text-cyan-400 flex-shrink-0" />
          <span className="text-[11px] text-cyan-300/80">Free transfer — no coins required</span>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-[10px] text-white/40 mb-1.5 uppercase tracking-wider font-mono">
            ...
            <input type="text" value={username} onChange={e => setUsername(e.target.value)}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-white/30 transition-colors font-mono" />
          </div>
          <div>
            <label className="block text-[10px] text-white/40 mb-1.5 uppercase tracking-wider font-mono">
              Quantity {item.quantity > 1 ? `(max ${item.quantity})` : ""}
          ...
          <button onClick={() => canSubmit && !isPending && onSubmit(parsedQty, username.trim())}
            disabled={!canSubmit || isPending}
            className="w-full py-3 rounded-xl bg-cyan-500/15 hover:bg-cyan-500/25 disabled:opacity-30 border border-cyan-500/30 text-sm font-bold text-cyan-300 transition-colors flex items-center justify-center gap-2">
            {isPending
              ? <><Loader2 className="w-4 h-4 animate-spin" /> Transferring...</>
              : <><Send className="w-4 h-4" /> Send to {username || "..."}</>}
          </button>
        </div>
      </motion.div>
    </motion.div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        ...
        <p className="text-sm text-white/50 mb-5">
          Buy <span className="text-white font-medium">{meta.name}</span> x{listing.quantity} for{" "}
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={isPending || !canAfford}
...
export default function Merchant() {
  ...
  const [tab, setTab]                     = useState<"browse" | "sell" | "transfer" | "my-listings">("browse");
  ...
  const transferMut = useMutation({
    mutationFn: async ({ item_type, quantity, recipient_username }: any) => {
      const { data: recipient, error: lookupError } = await supabase
        .from("profiles")
        .select("id")
        .eq("username", recipient_username)
        .single();
      if (lookupError || !recipient) throw new Error("User not found");
    ...
    onSuccess: () => { invalidate(); setTransferModal(null); showToast("Item transferred successfully!"); },
    onError: (e: any) => showToast(e.message ?? "Transfer failed", false),
  });
  ...
  const TABS = [
    ...
    { key: "my-listings", label: "Mine",     icon: Package       },
  ];

  const showViewToggle = tab === "browse" || tab === "my-listings";
  ...
  return (
    <div className="relative min-h-[100dvh] w-full bg-black text-white overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-5">
        ...
        {/* Tabs + view toggle */}
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-1 p-1 rounded-xl border border-white/10 bg-white/5">
          ...
          {showViewToggle && (
            <div className="flex items-center gap-1 p-1 rounded-xl border border-white/10 bg-white/5">
              <button onClick={() => setViewMode("list")}
                className={`p-1.5 rounded-lg transition-all ${viewMode === "list" ? "bg-white/15 text-white" : "text-white/30 hover:text-white/60"}`}>
                <LayoutList className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setViewMode("grid")}
                className={`p-1.5 rounded-lg transition-all ${viewMode === "grid" ? "bg-white/15 text-white" : "text-white/30 hover:text-white/60"}`}>
        ...
        {tab === "browse" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {!loadingListings && listings.length === 0 && (
              <div className="text-center py-16 text-white/30 text-sm">No active listings yet. Be the first to sell!</div>
            )}
            <AnimatePresence>
              {viewMode === "list" ? (
                <div className="space-y-3">
                  {listings.map((listing: any) => (
                    <ListingCard key={listing.id} listing={listing}
                      isMine={listing.seller_id === userId} coinBalance={coinBalance}
                      onBuy={setConfirmBuy} onCancel={(id: string) => cancelListingMut.mutate(id)}
                      isPendingBuy={buyListingMut.isPending} isPendingCancel={cancelListingMut.isPending} />
                  ))}
                </div>
              ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            ...
            <AnimatePresence>
              {sellableItems.map((item: any) => {
                const meta = getItemMeta(item.item_type);
                return (
                  <motion.div key={item.item_type} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <button onClick={() => setSellModal(item)}
        ...
        {tab === "transfer" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            <div className="flex items-center gap-2 px-4 py-3 rounded-2xl border border-cyan-500/20 bg-cyan-500/5">
              <ArrowRightLeft className="w-4 h-4 text-cyan-400 flex-shrink-0" />
              <p className="text-xs text-cyan-300/70 leading-relaxed">
                Send items directly to another player for free. No coins needed — just their exact username.
              </p>
            </div>
            {loadingInventory && (
              <div className="flex items-center justify-center py-16">
            ...
            {!loadingInventory && sellableItems.length === 0 && (
              <div className="text-center py-16 text-white/30 text-sm">Your inventory is empty. Nothing to transfer yet.</div>
            )}
            <AnimatePresence>
              {sellableItems.map((item: any) => {
                const meta = getItemMeta(item.item_type);
                return (
                  <motion.div key={item.item_type} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <button onClick={() => setTransferModal(item)}
                      className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 text-xs text-cyan-300 font-medium transition-colors">
        ...
        {tab === "my-listings" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {myListings.length === 0 && (
              <div className="text-center py-16 text-white/30 text-sm">You have no active listings.</div>
            )}
            <AnimatePresence>
              {viewMode === "list" ? (
                <div className="space-y-3">
                  {myListings.map((listing: any) => (
                    <ListingCard key={listing.id} listing={listing}
                      isMine={true} coinBalance={coinBalance}
                      onBuy={() => {}} onCancel={(id: string) => cancelListingMut.mutate(id)}
                      isPendingBuy={false} isPendingCancel={cancelListingMut.isPending} />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
      ...
      <AnimatePresence>
        {sellModal && (
          <SellModal item={sellModal} onClose={() => setSellModal(null)}
      ...
      <AnimatePresence>
        {transferModal && (
          <TransferModal item={transferModal} onClose={() => setTransferModal(null)}
            onSubmit={(qty, username) => transferMut.mutate({
              item_type: transferModal.item_type, quantity: qty, recipient_username: username,
            })}
      ...
      <AnimatePresence>
        {confirmBuy && (
          <BuyModal listing={confirmBuy} coinBalance={coinBalance}
