...
const TWEET_QUESTS = [
  { id: "tweet-1", url: "https://x.com/i/status/2049489255022375413", actions: [{ id: "t1-like", label: "Like", points: 25 }, { id: "t1-comment", label: "Comment", points: 50 }, { id: "t1-retweet", label: "Retweet", points: 25 }] },
  { id: "tweet-2", url: "https://x.com/i/status/2048723259009462774", actions: [{ id: "t2-like", label: "Like", points: 25 }, { id: "t2-comment", label: "Comment", points: 50 }, { id: "t2-retweet", label: "Retweet", points: 25 }] },
  { id: "tweet-3", url: "https://x.com/i/status/2046358679272706428", actions: [{ id: "t3-like", label: "Like", points: 25 }, { id: "t3-comment", label: "Comment", points: 50 }, { id: "t3-retweet", label: "Retweet", points: 25 }] },
];
...
function CountdownAction({ actionId, label, points, openUrl, userId, onAwarded, layout = "column" }: {
  ...
  const award = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.rpc("increment_score", { p_user_id: userId, p_points: points });
  ...
  const start = () => {
    ...
    timerRef.current = setInterval(() => {
      setSecs(s => {
        if (s <= 1) { clearInterval(timerRef.current!); award.mutate(); return 0; }
        return s - 1;
      });
    }, 1000);
  };
  ...
  if (layout === "row") {
    if (state === "done") return (
      <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-semibold flex-shrink-0">
        <CheckCircle2 className="w-4 h-4" /> +{points} pts
      </div>
    );
  ...
  if (state === "done") return (
    <div className="flex flex-col items-center gap-1 py-3 bg-green-500/5">
      ...
      <span className="text-[9px] text-white/25 uppercase tracking-wide">Claimed</span>
    </div>
  );
  if (state === "counting") return (
...
export default function Socials() {
  ...
  return (
    <div className="relative min-h-[100dvh] w-full overflow-hidden bg-black text-white">
      ...
      <nav className="relative z-30 flex items-center justify-between px-5 sm:px-10 py-4 border-b border-white/8 bg-black/20 backdrop-blur-md">
        <button onClick={() => setLocation("/")} className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/15 bg-white/5 hover:bg-white/10 transition-colors text-white/70 hover:text-white text-sm">
          <ArrowLeft className="w-4 h-4" /> Back
        </button>
        <span className="text-sm font-bold tracking-[0.15em] uppercase text-white/70">Socials</span>
            ...
            {TWEET_QUESTS.map((tweet, i) => (
              <motion.div key={tweet.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 + i * 0.08, type: "spring", damping: 22 }}
                ...
                <a href={tweet.url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 px-4 py-3 border-b border-white/8 hover:bg-white/5 transition-colors group">
                  <Twitter className="w-4 h-4 text-sky-400 flex-shrink-0" />
                  <span className="text-sm text-white/60 flex-1 min-w-0 truncate font-mono">Post #{tweet.url.split("/").pop()}</span>
                  <ExternalLink className="w-3.5 h-3.5 text-white/20 group-hover:text-white/50 flex-shrink-0" />
                </a>
