    ...
    <div className="relative min-h-[100dvh] w-full overflow-hidden bg-black text-foreground">
      ...
      <div className="relative z-10 min-h-[100dvh] flex flex-col">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="gate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }} className="flex-1 flex items-center justify-center p-6">
              <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", damping: 20, delay: 0.1 }} className="w-full max-w-sm">
                <div className="text-center mb-8">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">Private beta — invite only</p>
                </div>
              ...
              <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", damping: 20, delay: 0.1 }} className="w-full max-w-sm">
                <div className="text-center mb-8">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Redeem your invite to unlock the protocol.
                  </p>
                </div>
                ...
                <form onSubmit={handleCodeSubmit} className="space-y-4">
                  <Input
                    ...
                    maxLength={8}
                    className="h-14 text-center text-lg font-mono tracking-[0.25em] uppercase bg-black/50 border-border/60 backdrop-blur-md"
                  ...
                  <Button type="submit" className="w-full h-12 text-sm font-semibold" disabled={validateMutation.isPending || code.length < 4}>
          ...
          {phase === "validating" && (
            <motion.div key="validating" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-2xl font-semibold tracking-tight">Code accepted</motion.h2>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="mt-3 text-sm text-muted-foreground">Unlocking the protocol…</motion.p>
              </div>
            </motion.div>
          )}

          {/* ── CHOICE: Pledge or Rabel ──────────────────────────────────── */}
          {phase === "choice" && (
            <motion.div key="choice" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex items-center justify-center p-6">
              <div className="w-full max-w-3xl">
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  <motion.button initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, type: "spring" }} whileHover={{ scale: 1.02, y: -4 }} whileTap={{ scale: 0.98 }} onClick={() => setLocation("/connect")} className="group relative text-left rounded-2xl border border-border/50 bg-black/40 backdrop-blur-md p-8 hover:bg-black/60 hover:border-indigo-500/40 transition-colors">
          ...
          {/* ── RABEL: Element picker + guild name ──────────────────────── */}
          {phase === "rabel" && (
