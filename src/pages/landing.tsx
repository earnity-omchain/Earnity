...
import { Loader2, Shield, Swords, CheckCircle2, AlertCircle, Sparkles, ArrowLeft } from "lucide-react";
...
export default function Landing() {
  ...
  // Check if user already has a valid session + redeemed code
  useEffect(() => {
    const checkSession = async () => {
      const { data } = await supabase.auth.getSession();
      if (data.session) {
        // User is logged in, check if they have a guild or invite code used
        const { data: profile } = await supabase
          .from("profiles")
          .select("guild_id, invite_code_used")
        ...
        if (profile?.guild_id) {
          ...
          // They redeemed a code but haven't joined/created guild yet
          setPhase("choice");
        }
      }
    };
  ...
  const validateMutation = useMutation({
    mutationFn: async (accessCode: string) => {
      ...
      // Otherwise just validate (redeem after they create profile via Discord)
      return api.validateInviteCode(accessCode);
    },
    onSuccess: () => {
      setPhase("validating");
  ...
  // Pledge redirects to connect (Discord OAuth + profile creation)
  useEffect(() => {
              ...
              >
                <div className="text-center mb-8">
                  <motion.div
                    ...
                    className="w-16 h-16 mx-auto mb-5 rounded-2xl bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center shadow-lg shadow-primary/20"
                  >
                    <Shield className="w-8 h-8 text-primary-foreground" />
                  </motion.div>
                  <h1 className="text-3xl font-bold tracking-tight">Earnity</h1>
                  <p className="mt-2 text-sm text-muted-foreground">Enter your access code to enter the protocol</p>
                </div>
                ...
                <form onSubmit={handleCodeSubmit} className="space-y-4">
                  <div>
                    <Input
                      ...
                      maxLength={8}
                      className="h-14 text-center text-lg font-mono tracking-[0.25em] uppercase bg-black/50 border-border/60 backdrop-blur-md focus:ring-2 focus:ring-primary/30"
              ...
              <div className="w-full max-w-3xl">
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  ...
                  <motion.button
                    ...
                    className="group relative text-left rounded-2xl border border-border/50 bg-black/40 backdrop-blur-md p-8 hover:bg-black/60 hover:border-indigo-500/40 transition-colors"
                  >
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Join an existing guild. Create your profile, link Discord, and bind your wallet to swear allegiance.
                    </p>
                  ...
                  <motion.button
                    ...
                    className="group relative text-left rounded-2xl border border-border/50 bg-black/40 backdrop-blur-md p-8 hover:bg-black/60 hover:border-orange-500/40 transition-colors"
                  >
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Forge your own guild. Choose your elemental seal, declare your guild name, and establish a new faction.
                    </p>
              ...
              <div className="w-full max-w-xl">
                ...
                <div className="relative w-full max-w-[380px] aspect-square mx-auto mb-8">
                  ...
                  <motion.div
                    ...
                    className="absolute inset-10 rounded-full border border-border/10"
                  />
                  ...
                  {ELEMENTS.map((el, i) => {
                    ...
                    return (
                      <motion.button
                        ...
                        animate={{
                          opacity: selectedElement && !isSelected ? 0.35 : 1,
                        ...
                        <div className={`rounded-xl border ${selectedEl?.border || "border-border"} bg-black/50 backdrop-blur-md p-5 space-y-4`}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium">
                              {selectedEl?.name} bond selected
                            </span>
                          </div>

                          <div className="space-y-2">
                            <Label htmlFor="guild" className="text-xs uppercase tracking-wider text-muted-foreground">
                              Guild Name
                            </Label>
                            <Input
                              id="guild"
                              ...
                              className="bg-black/40 border-border/60 h-11"
                          ...
                          <Button
                            ...
                            disabled={createGuildMutation.isPending || guildName.trim().length < 2}
                          >
                          ...
                          {createGuildMutation.isError && (
                            <p className="text-sm text-destructive text-center">
                              {createGuildMutation.error instanceof Error ? createGuildMutation.error.message : "Failed to create guild"}
                            </p>
                          )}
                        </div>
                      </form>
